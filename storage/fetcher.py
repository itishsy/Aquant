...
def fetch_all():
    code_dict = db.query("SELECT `code`, IFNULL(`updated`,null) AS  updated FROM `code_dict` WHERE `latest` = 0")
    print(code_dict)
        ...
        for klt in config.klt:
            ...
            try:
                ...
                db.execute(db.get_connect(),
                           "UPDATE `code_dict` SET `updated` = '{}' WHERE `code` = '{}'".format(updated, code))
            begin_date = None
